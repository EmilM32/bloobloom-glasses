...
<template>
  <div ref="scrollComponent">
    <div
      class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-[1px] bg-black border-black border-b"
    >
      <SingleGlassItem
        v-for="item in collectionStore.glasses"
        :key="`glass-item-${item.id}`"
        :title="item.name"
        :variants="item.glass_variants"
        class="bg-white"
      />
    </div>
