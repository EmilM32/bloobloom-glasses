...
const filters = computed(() =>
  props.activeFilter === AvailableFilters.Colour ? filtersStore.colours : filtersStore.shapes
)
const addFilter = computed(() =>
  props.activeFilter === AvailableFilters.Colour ? filtersStore.addColour : filtersStore.addShape
)
const removeFilter = computed(() =>
  props.activeFilter === AvailableFilters.Colour
    ? filtersStore.removeColour
    : filtersStore.removeShape
)
...
const toggleFilter = (item: string) => {
  const value =
    props.activeFilter === AvailableFilters.Colour
      ? Colour[item as keyof typeof Colour]
      : Shape[item as keyof typeof Shape]
...
<template>
  <div class="md:flex gap-4 md:justify-evenly pt-5 pb-5 flex-wrap grid grid-cols-2 text-center">
    <span
      ...
      :class="{ underline: showUnderline(item) }"
