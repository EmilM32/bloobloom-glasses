...
watch(isOutsideNavigationButtonRef, () => { toggleNav() });
watch(isOutsideNavigationRef, () => { toggleNav() });
...
<template>
  <div>
    <div class="fixed top-0 left-0 w-full bg-white h-14 border-black border-b flex items-center justify-between">
      <div ref="navigationButtonRef" class="ml-10 hover:underline cursor-pointer">
        MENU
      </div>
    </div>
    <div v-show="showNav" ref="navigationRef" class="fixed bg-white h-full flex md:w-auto w-full">
      <div class="flex-col md:w-[250px] w-full md:border-black md:border-r">
        <div v-for="item in menuItems" :key="item"
          ...
          :class="`${isItemActive(item) ? 'cursor-pointer' : 'cursor-not-allowed'}`" @mouseenter="setSubMenu(item)">
          {{ item }}
        </div>
      </div>
      <div v-show="showSubMenu" class="flex-col md:w-[250px] w-full md:border-black md:border-r">
        <div v-for="el in getSubMenuItems()" :key="el.id" @click="navigateTo(el.configuration_name)"
